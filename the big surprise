...
class BagLIFO implements IBag {
    ...
    public void put(IBag bagOfSurprises) {
        while (!bagOfSurprises.isEmpty()) {
            surprises.push(bagOfSurprises.takeOut());
        }
    }
...
class Candies implements ISurprise {
    ...
    public static Candies generate() {
        String[] candyTypes = {"chocolate", "jelly", "fruits", "vanilla"};
        Random random = new Random();
        int index = random.nextInt(candyTypes.length);
        int num = random.nextInt(10) + 1;
        return new Candies(num, candyTypes[index]);
    }
}
...
}//Loud applause to you... For it is in giving that we receive.
...
//Singing a nice song, full of joy and genuine excitement...
    @Override
    void giveWithPassion() {
